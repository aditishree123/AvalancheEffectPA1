...
def userinput():
    ...
    if len(datainp) == lenofdata and len(keyinp) == lenofkey:
        ...
        while len(datainp) != lenofdata:
            ...
            datainp = input(
                "Error in entered data. Enter the data (64 bits) to encrypt or decrypt seperated by space " "").strip().split()

        print("data entry accepted, data loaded succesfully")
        while len(keyinp) != lenofkey:
            print("length of key entered ", len(keyinp))
            keyinp = input(
                "Error in entered key. Enter the key (56 bits) to encrypt or decrypt seperated by space " "").strip().split()
        print("key entry accepted, key loaded succesfully")
...
def plot_hd(l, title):
    data = []
    # converting Δc in correct format to plot Δc against no. of rounds
    for j in range(0, 16):
        temp = []
        for i in range(0, 5):
            temp.append(l[i][j])
        data.append(temp)
    print(data)
    # plotting Δc vs rounds
    plt.boxplot(data, sym='r+')
    plt.title(title)
    plt.xlabel("Rounds")
    plt.ylabel("Δc(hamming distance)")
    plt.show()
...
def encryption(data, key):
    ...
    cipher = []
    for i in range(16):
        rdata = round(data, key16[i])
        cipher.append(rdata)

    # making left side right and right side left
    rdata = np.roll(rdata, 32)
    rdata = (permutation(rdata, 1))
    print("Encrypted data", rdata)
    return cipher


def main():
    # i) 5 different plaintexts
    f = open("diff_plaintext.txt")
    lines = f.readlines()
    # reading key from inputfile
    key = lines[1].strip().split()
    # original plaintext
    data = lines[3].strip().split()
    # list of ciphertexts after all 16 rounds obtained after calling encryption function
    org_cipher = encryption(data, key)
    hamDis = []
    # 5 different derived plaintexts with hamming distance 1 from original plaintext
    for i in range(4, 9):
        data = lines[i].strip().split()
        der_cipher = encryption(data, key)
        temp = []
        # Δc between original and derived ciphertexts of each round
        for i in range(0, 16):
            temp.append(hamming_distance(org_cipher[i], der_cipher[i]))
        hamDis.append(temp)
    # plot Δc against no. of rounds
    plot_hd(hamDis, "5 different plaintexts")

    # ii) 5 different Hamming distances
    f = open("diff_hammingDistance.txt")
    lines = f.readlines()
    # reading key from input file
    key = lines[1].strip().split()
    # original plaintext
    data = lines[3].strip().split()
    # list of ciphertexts after all 16 rounds obtained after calling encryption function
    org_cipher = encryption(data, key)
    hamDis = []
    # 5 different derived plaintexts with different hamming distances from original plaintext
    for i in range(4, 9):
        data = lines[i].strip().split()
        der_cipher = encryption(data, key)
        temp = []
        ...
        hamDis.append(temp)
    # plot Δc against no. of rounds
    plot_hd(hamDis, "5 different HammingDistances")

    # iii) 5 different secret keys
    f = open("diff_keys.txt")
    lines = f.readlines()
    # reading plaintext from input file
    data = lines[1].strip().split()
    # original key
    key = lines[3].strip().split()
    # list of ciphertexts after all 16 rounds obtained after calling encryption function
    org_cipher = encryption(data, key)
    hamDis = []
    # 5 different derived keys with hamming distance 1 from original key
    for i in range(4, 9):
        key = lines[i].strip().split()
        der_cipher = encryption(data, key)
        temp = []
        # Δc between original and derived ciphertexts of each round
        for i in range(0, 16):
        ...
        hamDis.append(temp)
    # plot Δc against no. of rounds
    plot_hd(hamDis, "5 different keys")
